...
public class MyContextView : MVCSContext
{
    ...
    override protected void mapBindings()
    {
		...
		commandBinder.Bind (GameEvents.ON_UPDATE_POSITION_PLAYER).To<UpdatePositionPlayer> ();
		commandBinder.Bind (GameEvents.ON_TOUCH_DESECHAR).To<OnDiscardItemCommand> ();
		commandBinder.Bind (GameEvents.ON_GRAP_COLLECTABLE).To<OnGrapCollectable> ();
